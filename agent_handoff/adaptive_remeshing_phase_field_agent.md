...
- P3-S-H1 execution-policy correction is prepared but not yet authorized: PBS and submitter now use `entry_imfdfkmq`; a valid returned PBS job ID atomically consumes the one-shot authorization, while failed/invalid submission leaves it unused. Fifteen synthetic unittest cases pass. Workstation pytest is unavailable (M-080); cluster pytest is required after commit synchronization. Scientific inputs, instrumentation, Abaqus 2023, CPU1, 16 GB and 00:30:00 remain unchanged.
