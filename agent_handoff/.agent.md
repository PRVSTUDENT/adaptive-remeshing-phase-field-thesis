...
1. Prepare, validate, commit, push, synchronize, and submit exactly three serial HPC jobs for Molnar `lc=0.015 mm` h-convergence: H0, H1, H2-PUB.
2. Do not submit length-scale, increment-sensitivity, MISESERI, remeshing, multi-CPU, GPU, or any fourth mesh job.
...
6. Treat the HPC trivial technical gate as complete: environment, Abaqus license, user-subroutine compile/link, trivial Standard analysis, ODB creation, and `UEXTERNALDB` callback evidence have passed.
7. Only after benchmark reproduction is stable and explicitly authorized, start the MISESERI pre-refinement milestone.
