...
- HPC account access and a queue/node/software snapshot are documented from `hpc_access_limits_report.txt` (captured 2026-07-14). SSH access was restored and verified on 2026-07-15; the HPC clone, home/scratch layout, Abaqus/Intel module probe, PBS queue inspection, and prepared-but-not-submitted smoke script are now documented. Production submissions remain blocked until the environment/user-subroutine smoke job is explicitly approved and passes; Gate A3 remains an independent scientific blocker.
...
7. Review and commit/push the local HPC documentation updates if approved, then run the small serial `testq` environment/user-subroutine smoke gate only after explicit submission approval.
