...
C     ==================================================================
     ...
     3 VNI(2,NDOFEL),ULOC(2),PHASENOD(NNODE)
       ...
       CHARACTER*512 F14OUT,F14JOB,F14LOG
       INTEGER F14LO,F14LJ,F14IOS
...
C      
       ...
       IF (JELEM.LT.1.OR.JELEM.GT.N_ELEM) THEN
        WRITE(7,*) 'F11 BOUNDS PHASE',JELEM,N_ELEM
        CALL XIT
       ENDIF
C     ==================================================================
...
C     Consistent quadrature-point penalty for d_(n+1) >= d_n.
     ...
     1   (DP(1)*DP(1)+DP(2)*DP(2))
        ...
        IF (JELEM.EQ.1.AND.INPT.EQ.1.AND.STEPITER.LE.20) THEN
         CALL GETOUTDIR(F14OUT,F14LO)
         CALL GETJOBNAME(F14JOB,F14LJ)
         IF (F14LO.LE.0.OR.F14LJ.LE.0) CALL XIT
         F14LOG=F14OUT(1:F14LO)//'/'//F14JOB(1:F14LJ)//
     1    '_uel_entry.log'
         OPEN(UNIT=98,FILE=F14LOG,STATUS='UNKNOWN',
     1    POSITION='APPEND',ACTION='WRITE',IOSTAT=F14IOS)
         IF (F14IOS.NE.0) CALL XIT
         WRITE(98,*) 'F14_UEL_ENTRY',F14JOB(1:F14LJ),
     1    F14OUT(1:F14LO),KSTEP,KINC,JELEM,INPT,
     2    TIME(1),TIME(2),DTIME,STEPITER
         CLOSE(98)
        ENDIF
C
C     ==================================================================
...
C     ==================================================================
      ...
      IF (NELEMAN.LT.1.OR.NELEMAN.GT.N_ELEM) THEN
       WRITE(7,*) 'F11 BOUNDS DISP',JELEM,N_ELEM,NELEMAN
       CALL XIT
      ENDIF
...
C
       ...
       IF (NELEMAN.LT.1.OR.NELEMAN.GT.N_ELEM) THEN
        WRITE(7,*) 'F11 BOUNDS UMAT',NOEL,N_ELEM,NELEMAN,NPT
        CALL XIT
       ENDIF
       IF (NPT.LT.1.OR.NPT.GT.4) THEN
        WRITE(7,*) 'F11 BOUNDS NPT',NOEL,NPT
        CALL XIT
       ENDIF
       IF (NPT.EQ.3) THEN
        NPT=4
