...
- D3A0/D3A1 tested reuse of the existing H0 ODB `1376154.mmaster02`. The corrected CAE/ODB-only extraction attempt `1376879.mmaster02` selected `U2=0.003000000026077032 mm`, extracted 15720 element/IP rows with `target_ip_coverage=1.0`, finite `SDV15`/`SDV16`, `max_d=0.08412302285432816`, `max_H=0.0512588769197464`, checkpoint `RF2=0.39450356364250183`, and `RF2/H0_peak=0.5421925638518931`. The source ODB lacks `ALLIE`, `ALLSE`, and `ALLWK`, so independent energy reconstruction was required before D3A acceptance.
- D3A-E R0 job `1376885.mmaster02` is preserved as `stage_d3a_energy_reconstruction_fail_parser_scope`; the first reconstruction used a scope-insensitive input parser that mixed `Part-1` node labels with the assembly reference-point namespace.
- D3A-E R1 passed with classification `stage_d3a_energy_reconstruction_pass`: 3930 physical elements, 15720 integration points, non-positive detJ count `0`, minimum detJ `2.829135024804933e-06`, and relative energy residual `0.012586306767288707`. D3A is closed as `stage_d3a_checkpoint_pass_independent_energy_reconstruction`; `D3A.ok` exists, and D3A2 nonmatching package construction is next.
- D3A2 passed locally with classification `stage_d3a2_transfer_package_pass`. The deterministic nonmatching target has 6601 nodes, 6400 Q4 physical elements, 25600 IPs, an actual split notch from `x=-0.5` to `x=0.0` with 40 duplicated open-face node pairs and a shared tip, node/IP coverage `1.0`, non-positive detJ count `0`, predicted energy relative jump `0.015379624558651227`, and unmapped state count `0`. `D3_PACKAGE.ok` exists with `solver_job_submitted=false`; D3A3 is the next serial ingestion/equilibration/release-hold gate.
