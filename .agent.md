...
- P3-S hardening is preparation-only: `runs/hpc/stage_p/p3s_serial_diagnostic/P3S_AUTHORIZATION.json` explicitly sets submission false and all downstream authorities false. The lane now uses login-side immutable staging/hashes, no compute-node Git, a failure-safe finalizer, lightweight evidence allowlist, strict technical/scientific gates, increment-sequence evidence, and synthetic failure tests. The Stage P thesis text remains a standalone draft and is not included in the closeout build. No job was submitted.
