...
- P3-SM1T preparation is complete and exactly one guarded serial execution is
  authorized: an isolated
  `GETTHREADID()` call is bracketed by before/after markers inside the existing
  controlled UEL callback. The deck/include match accepted P3-SM0 byte-for-byte
  and scientific calculations are unchanged. Authorization is true at 0/1
  used with no retry; P3-SM1R is design-only, and P3-T4/MPI/hybrid/P4/H1/D3D-A1
  reopening/D3E remain blocked. No Abaqus or PBS job was submitted.
