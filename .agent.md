...
- HPC account access and a queue/node/software snapshot are documented from `hpc_access_limits_report.txt` (captured 2026-07-14). SSH access was restored and verified on 2026-07-15; the HPC clone, home/scratch layout, Abaqus/Intel module probe, PBS queue inspection, and serial `testq` environment smoke attempts are now documented. Job `1374529.mmaster02` ran on `mnode098.cluster` but failed at `ifx --version` with Intel error `#10417`; repaired rerun job `1374530.mmaster02` loaded `gcc/11.4.0` first but failed before compiler diagnostics because `git` was not available in the compute-node batch PATH. This second failure is classified as `batch_metadata_dependency` and does not establish whether `ifx` works after the GCC repair. Production submissions remain blocked until this environment smoke is repaired and passes; Gate A3 remains an independent scientific blocker.
...
7. Repair the HPC environment smoke script so compute-node repository revision capture is robust, likely by loading `git/gcc/11.4.0/2.51.2` before calling `git` or by passing the revision from submission time, then rerun the small serial `testq` environment smoke only after explicit submission approval.
