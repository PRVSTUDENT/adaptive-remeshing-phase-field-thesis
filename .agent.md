...
- WP7-F1 final documentation closeout is complete with classification
  `wp7_final_closeout_reconciliation_complete`. Stage-A reports are frozen as
  `frozen_with_residual_scientific_limitations`; the Stage-B results and
  execution/failure report pair is created and frozen; the checklist
  validator, 100 repository unit tests, five standalone validation scripts,
  expanded reproducibility audit, and final LaTeX builds pass. The assembled
  thesis is 30 pages and both Stage-B reports are 3 pages. WP6 remains
  externally blocked, Stage P remains scientifically closed, and no
  Abaqus/PBS job is authorized. Submission-package review is next. Evidence:
...
- Stage P execution is closed as technically inconclusive for threaded
  safety. P3-SM0 establishes the accepted serial minimal-callback baseline;
  P3-SM1TC and P3-SM1R qualify only their controlled one-rank/one-thread
  identifier calls; P3-T4 produced no threaded evidence. The earlier D2C
  equality is positive, case-specific repeatability evidence, not general
  COMMON/SAVE thread-safety proof. MPI and hybrid safety remain unqualified.
  No Abaqus/PBS job is authorized; P3-T4-J2, retries or replacements, MPI,
  hybrid, P4, production H1, D3D-A1 reopening, and D3E remain blocked.
  Evidence: `docs/decisions/STAGE_P_FINAL_SCIENTIFIC_CLOSURE.md` and
  `docs/decisions/P3T4_HEADER_SCOPE_FORENSIC_CORRECTION.md`.
- P3-T4 job `1378242.mmaster02` consumed its one authorization from revision
  `71d42487cbf9165b9b7aca2b6e34fcae7a0b77b9` and closed as
  `stage_p3t4_threaded_fail_compile` (scheduler/solver `10/1`). The aggregate
  Abaqus utility header was placed before the first subroutine, outside a
  valid Fortran scoping unit. No linking, callbacks, threading diagnostics,
  ODB, or scientific comparison occurred. No retry is authorized; all
  downstream routes remain blocked.
- P3-T4 preparation is released only for the bounded one-rank/four-thread
  eight-element characterization. Its accepted P3-SM0 science is unchanged;
  qualified rank/thread calls occur only in UEL/UMAT diagnostic helpers, and
  mutexes protect diagnostic metadata rather than scientific accesses.
  Execution authorization remains false at 0/1 and no job has been submitted.
  MPI, hybrid, P4, production H1, D3D-A1 reopening, and D3E remain blocked.
- P3-SM1R job `1378241.mmaster02` passed from authorization revision
  `4e941ffa7740a4c8050277351f25bcf392f8cd98`: scheduler/solver exit 0,
  GETRANK markers 3/3, returned process IDs `[0,0,0]`, unique `[0]`, readable
  ODB, 32/32 state records, and 13 increments. Authorization is consumed 1/1;
  no retry occurred. This qualifies only the controlled one-rank/one-thread
  UEL call. P3-T4 and all downstream routes remain blocked.
- The GETRANK route decision selects offline P3-SM1R preparation using only
  `CALL GETRANK(KPROCESSNUM)` inside controlled UEL. Its authorization remains
  false at 0/1, retry is disabled, and no Abaqus/PBS job has been submitted.
  The deck and transfer table are byte-identical to P3-SM0 and the scientific
  model is unchanged. P3-T4 and every downstream route remain blocked.
- P3-SM1T job `1378239.mmaster02` consumed the only authorization (1/1) from
  revision `dd24a3f64e78cdb104c2946947801947350d1c17` and failed at the isolated
  utility call. Compile/link/input passed; the UEL before marker was written,
  then `libstandardU.so` reported unresolved symbol `getthreadid_`. After
  count 0, returned IDs empty, signal 11 absent. Classification:
  `stage_p3sm1t_threadid_serial_fail_identifier`. No retry is authorized;
  P3-SM1R and P3-T4/MPI/hybrid/P4/H1/D3D-A1 reopening/D3E remain blocked.
- P3-I1 offline audit found the installed Abaqus 2023 Fortran header declares
  `get_thread_id()`, not `GETTHREADID()`. Runtime evidence is consistent with
  `get_thread_id_`; the closed job requested unresolved `getthreadid_`.
  Classification: `stage_p3_identifier_interface_partially_confirmed`.
  P3-SM1TC is prepared from P3-SM0 as a new corrected-interface lane but is
  unauthorized at 0/1. No Abaqus/PBS job was submitted during preparation.
- P3-SM1TC job `1378240.mmaster02` passed from revision
  `b072e32f322b440729318e06f2c2ab72d041fc12`: scheduler/solver exit 0,
  matched markers 3/3, returned IDs `[0,0,0]`, unique `[0]`, no signal 11,
  readable ODB, 32/32 state records, and 13 increments. Authorization is
  consumed 1/1; no retry occurred. GETRANK route review is next; P3-SM1R,
  P3-T4 and all downstream routes remain blocked.
