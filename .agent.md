...
- Stage D3A3-R2-R1 pathfix is prepared locally as a bounded compile/datacheck retry. The only generated-source change is `UEXTERNALDB` resolving `d3_transfer_h.dat` via `GETOUTDIR` and `HFILE(1:LHFILE)` instead of a relative open. Path audit classification: `stage_d3a3_r2_r1_path_audit_pass`; evidence is under `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_compile_r2_r1/`; runtime-H SHA256 remains `4689ea5c10c0972e69ba46f8676a326c8b011b98faa8031c7c26cfb218607cd9`. Full D3A3-R2, D3D, and D3E remain blocked until the corrected datacheck creates and commits `D3A3_R2_COMPILE.ok`.
