...
runtime_dir_env = os.environ.get('F38_RUNTIME_DIR', '').strip()

if not runtime_dir_env:
    raise RuntimeError(
        'F38_RUNTIME_DIR is required because Abaqus/CAE noGUI may execute '
...
if not os.path.isdir(runtime_dir):
    raise RuntimeError(
        'F38_RUNTIME_DIR does not exist: {0}'.format(runtime_dir)
    )
...
audit_data = {
    ...
    'runtime_dir_source': 'F38_RUNTIME_DIR',
    'runtime_dir_exists': os.path.isdir(runtime_dir),
    'runtime_dir_on_sys_path': runtime_dir in sys.path,
    'bootstrap_passed': True
}

audit_path = os.environ.get('F38_INVOCATION_AUDIT', 'CAE_INVOCATION_CONTEXT_AUDIT.json').strip()
try:
