# Python 2 and 3 compatible Abaqus model builder script for F34
# Fail-closed Abaqus/CAE Python script with exact API signatures,
# environment variable argument transport (F34_SOURCE_DECK, F34_OUTPUT_INPUT, F34_GEOMETRY_AUDIT),
...
def main():
    ...
    source_deck_path = os.environ.get('F34_SOURCE_DECK', 'runtime/source_deck.inp')
    output_inp_path = os.environ.get('F34_OUTPUT_INPUT', 'M2RMPROV2.inp')
    audit_json_path = os.environ.get('F34_GEOMETRY_AUDIT', 'GEOMETRY_BACKED_MODEL_AUDIT.json')

    if not source_deck_path or not output_inp_path or not audit_json_path:
        print("ERROR: Mandatory environment variables F34_SOURCE_DECK, F34_OUTPUT_INPUT, F34_GEOMETRY_AUDIT must be set.")
        sys.exit(1)
    ...
    if source_part_name not in m.parts:
        print("ERROR: Imported model has no Part-1.")
        sys.exit(1)
    ...
    write_audit('SOURCE_MODEL_INVENTORY.json', {'material_present': 'Elastic_Matrix' in m.materials, 'section_present': 'Section-1' in m.sections, 'step_present': 'Step-1' in m.steps, 'assembly_sets': sorted(existing_set_names)})
    ...
    final_audit = {
        "protocol_version": 1,
        "task_id": "F34-M2RMBUILD9-OFFLINE-REPAIR",
    ...
    if not final_audit['contract_pass']:
        print("ERROR: F34 live runtime contract failed.")
        sys.exit(1)
