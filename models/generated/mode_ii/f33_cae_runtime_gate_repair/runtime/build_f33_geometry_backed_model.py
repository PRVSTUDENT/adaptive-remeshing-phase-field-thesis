# Python 2 and 3 compatible Abaqus model builder script for F33
# Fail-closed Abaqus/CAE Python script with exact API signatures,
# environment variable argument transport (F33_SOURCE_DECK, F33_OUTPUT_INPUT, F33_GEOMETRY_AUDIT),
...
def main():
    ...
    source_deck_path = os.environ.get('F33_SOURCE_DECK', 'runtime/source_deck.inp')
    output_inp_path = os.environ.get('F33_OUTPUT_INPUT', 'M2RMPROV1.inp')
    audit_json_path = os.environ.get('F33_GEOMETRY_AUDIT', 'GEOMETRY_BACKED_MODEL_AUDIT.json')

    if not source_deck_path or not output_inp_path or not audit_json_path:
        print("ERROR: Mandatory environment variables F33_SOURCE_DECK, F33_OUTPUT_INPUT, F33_GEOMETRY_AUDIT must be set.")
        sys.exit(1)
    ...
    final_audit = {
        "protocol_version": 1,
        "task_id": "F33-M2RMBUILD8-OFFLINE-REPAIR",
