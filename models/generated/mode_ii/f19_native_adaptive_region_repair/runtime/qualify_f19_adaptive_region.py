...
out=os.environ['F19_OUTPUT_DIR']; odbp=os.environ['F19_SOURCE_ODB']; deck=os.path.join(os.getcwd(),'source_deck.inp')
...
odb=None
try:
 m=mdb.ModelFromInputFile(name='F19_SOURCE',inputFileName=deck); p=m.parts[m.parts.keys()[0]]
 orphan=bool(getattr(p,'isMeshPart',False)); methods=[x for x in dir(m) if 'adapt' in x.lower() or 'remesh' in x.lower()]
 gp=m.Part2DGeomFrom2DMesh(name='F19_GEOMETRY_BACKED',part=p,featureAngle=20.0) if orphan and hasattr(m,'Part2DGeomFrom2DMesh') else None
 target=gp or p; geometry_backed=not bool(getattr(target,'isMeshPart',False))
 m.RemeshingRule(name='F19_MISESERI_RULE',stepName='Step-1',variables=(str('MISESERI'),),region=MODEL,sizingMethod=UNIFORM_ERROR,errorTarget=1.0,specifyMinSize=ON,minElementSize=0.001,specifyMaxSize=ON,maxElementSize=0.010,coarseningFactor=NOT_ALLOWED,refinementFactor=10)
 ...
 associated='F19_MISESERI_RULE' in m.remeshingRules and m.remeshingRules['F19_MISESERI_RULE'].region is not None
 ...
 wr('REMESH_RULE_MANIFEST.json',{'name':'F19_MISESERI_RULE','step':'Step-1','variables':['MISESERI'],'region_associated':associated})
