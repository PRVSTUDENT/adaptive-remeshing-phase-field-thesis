C ======================================================================
C User Subroutine UEL and UMAT for Abaqus: Stage F42C Scheme
C JTYPE = 1: 4-Node Quad Phase-Field UEL (U1)
C JTYPE = 2: 4-Node Quad Displacement UEL (U2)
C JTYPE = 3: 3-Node Triangle Phase-Field UEL (U3) - 3-IP + Centroid Slot 4
C JTYPE = 4: 3-Node Triangle Displacement UEL (U4) - 3-IP + Centroid Slot 4
C Subroutine UMAT: Facsimile Topology Dispatch (CPE4 NPT 1..4, CPE3 Centroid Slot 4)
C ======================================================================
...
C     ==================================================================
     ...
     4 XII_T(3,2),W_T(3)
       ...
       REAL*8 PHI_C, HIST_C, DEG_C, ENG_E_C, ENG_D_C
       REAL*8 EPS_C(3), STRESS_E_C(3), STRESS_D_C(3)
...
C       Dedicated Centroid Slot 4 Aggregation for U3
        ...
        USRVAR(PHYSIDX,16,4) = DBLE(PHYSIDX)
        USRVAR(PHYSIDX,17,4) = THREE
        USRVAR(PHYSIDX,18,4) = DBLE(KSTEP*100000 + KINC)
        RETURN
       ENDIF
...
C       Dedicated Centroid Physical Reconstruction for U4
        ...
        DO I=1,3
         STRESS_E_C(I) = ZERO
         DO J=1,3
          STRESS_E_C(I) = STRESS_E_C(I) + CMAT(I,J)*EPS_C(J)
         END DO
         STRESS_D_C(I) = DEG_C * STRESS_E_C(I)
        END DO
        ENG_E_C = HALF*(EPS_C(1)*STRESS_E_C(1) + EPS_C(2)*STRESS_E_C(2)
     1   + EPS_C(3)*STRESS_E_C(3))
        ENG_D_C = DEG_C * ENG_E_C

        DO I=1,3
         USRVAR(PHYSIDX,2+I,4) = EPS_C(I)
         USRVAR(PHYSIDX,5+I,4) = STRESS_E_C(I)
         USRVAR(PHYSIDX,8+I,4) = STRESS_D_C(I)
        END DO
        USRVAR(PHYSIDX,12,4) = ENG_E_C
        USRVAR(PHYSIDX,13,4) = ENG_D_C
        USRVAR(PHYSIDX,14,4) = DBLE(KSTEP)
        USRVAR(PHYSIDX,15,4) = DBLE(KINC)
        USRVAR(PHYSIDX,16,4) = DBLE(PHYSIDX)
        USRVAR(PHYSIDX,17,4) = THREE
...
C ======================================================================
C Subroutine UMAT: Facsimile Topology Dispatch (CPE4 NPT 1..4, CPE3 Slot 4)
C ======================================================================
     ...
     1 PROPS(NPROPS),COORDS(3),DSTRAN(NTENS)
      REAL*8 EMOD,ENU,EG,EG2,ELAM,TOPOMARK
      INTEGER NPHYS_VAL, PHYSIDX, NPT_READ, ITOPOLOGY
...
C      Unknown topology marker: Fail Closed
      ...
      IF (ITOPOLOGY.EQ.3) THEN
       ...
       IF (NTENS.GE.3) STRESS(3) = USRVAR(PHYSIDX,11,4)
      END IF
