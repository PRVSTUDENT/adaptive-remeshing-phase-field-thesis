...
- Stage D3A3-R1 is the bounded compiler-environment correction only: explicit `gcc/11.4.0`, `intel/2024.2.0`, and `abaqus/2023` module loads, `OMP_NUM_THREADS=1`, scratch execution, `ifort`/Abaqus environment capture, and separate evidence under `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_r1/`. The mesh, transfer package, input deck, Fortran logic, step definitions, and checkpoint displacement remain unchanged.
