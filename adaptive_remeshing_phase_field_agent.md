...
- Stage D2A transferred-state ingestion route is implemented and statically validated as `stage_d2a_executable_package_static_pass`: separate D2 source `src/state_transfer/d2_tiny_transfer_uel.for`, executable package `models/state_transfer/d2_tiny_transfer/executable/`, extractor `scripts/postprocessing/extract_d2_state_from_odb.py`, validator `scripts/validation/validate_d2_state_ingestion.py`, and D2A-only submit wrapper `scripts/hpc/stage_d2/submit_d2a_serial_ingestion.sh`. Submit D2A only; D2B/D2C/D2D remain blocked until upstream `.ok` markers exist.
