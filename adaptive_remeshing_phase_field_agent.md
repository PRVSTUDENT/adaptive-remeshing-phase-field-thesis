...
- Stage D2B serial continuation was submitted exactly once as job `1376819.mmaster02`. It failed technically with PBS `Exit_status=10`, solver exit `1`, and classification `stage_d2b_solver_fail`; the Abaqus message file reports `TOO MANY INCREMENTS NEEDED TO COMPLETE THE STEP` during the tiny continuation. Evidence is under `runs/hpc/stage_d2/d2b_serial_continuation/`. No `D2B.ok` exists. A corrective D2B deck is prepared with the same transfer values/labels/source route but `inc=50` for the release and continuation steps; do not resubmit without explicit authorization. D2C/D2D were not submitted and remain blocked.
