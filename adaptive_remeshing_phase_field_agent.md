...
- D3A3-R2 full ingestion/equilibration/release-hold lane is prepared but not yet submitted. New R2 scripts: `scripts/hpc/stage_d3/07_d3a3_r2_full_ingestion_hold.pbs` and `scripts/hpc/stage_d3/submit_d3a3_r2_full_ingestion_hold.sh`. They require the committed replay marker, stage runtime `d3_transfer_h.dat`, forbid `d3_transfer_table.inc`, run serial CPU1/16GB/02:00 with `OMP_NUM_THREADS=1`, preserve Abaqus outputs, run Abaqus-Python extraction, run strengthened validation, and create `D3A3.ok` only after gates pass. The deck requests global nodal `U, RF` output; extractor/validator now support target Q4 bulk-plus-AT2 energy reconstruction, phase/healing/H/RF/energy jump gates. Before submitting, commit and sync all checkouts, then submit exactly one full serial job.
