...
- Stage D3A3-R2-R2 counted-read correction is prepared as a compile/datacheck gate only. It keeps `GETOUTDIR`, preserves mesh/topology, input deck, runtime-H values/hash, materials, weak forms, layer offsets, steps, and checkpoint U2, and replaces the EOF-driven H loader with an exact `DO IREC=1,N_ELEM*NIP` loop plus explicit premature-EOF/read-error branches. Static audit classification: `stage_d3a3_r2_r2_counted_read_audit_pass`; login-node `ifort -extend-source` counted-reader smoke passed with 25600 records, first key `(1,1)`, last key `(6400,4)`, premature EOF false, read error false, duplicates 0, and missing keys 0. Evidence root: `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_compile_r2_r2/`. Submit exactly one serial counted-read datacheck only after smoke evidence is committed and pushed. Do not submit full D3A3-R2, D3D, or D3E until `D3A3_R2_COMPILE.ok` exists and is committed.
