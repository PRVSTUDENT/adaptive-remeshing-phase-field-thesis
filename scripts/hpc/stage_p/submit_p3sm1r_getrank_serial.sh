...
PREFLIGHT="${PROJECT_HOME}/scripts/validation/validate_p3sm1r_submission_preflight.py"
...
test -z "$(git status --porcelain -- \
  ...
  scripts/hpc/stage_p/06_p3sm1r_getrank_serial.pbs \
  scripts/hpc/stage_p/submit_p3sm1r_getrank_serial.sh \
  scripts/postprocessing/extract_p3sm1r_state.py \
...
mkdir -p "${STAGE_ROOT}"
cp "${PACKAGE}/P3SM1R_serial.inp" "${STAGE_ROOT}/P3SM1R_serial.inp"
cp "${PACKAGE}/p3sm1r_getrank_callback.for" "${STAGE_ROOT}/p3sm1r_getrank_callback.for"
cp "${PACKAGE}/d2_transfer_table.inc" "${STAGE_ROOT}/d2_transfer_table.inc"
cp scripts/postprocessing/extract_p3sm1r_state.py "${STAGE_ROOT}/extract_p3sm1r_state.py"
cp scripts/postprocessing/parse_p3sm1r_callback_log.py "${STAGE_ROOT}/parse_p3sm1r_callback_log.py"
cp scripts/validation/validate_p3sm1r_serial.py "${STAGE_ROOT}/validate_p3sm1r_serial.py"
cp scripts/validation/validate_p3sb_baseline_serial.py "${STAGE_ROOT}/validate_p3sb_baseline_serial.py"

DECK_SHA="$(sha256sum "${STAGE_ROOT}/P3SM1R_serial.inp" | awk '{print $1}')"
SOURCE_SHA="$(sha256sum "${STAGE_ROOT}/p3sm1r_getrank_callback.for" | awk '{print $1}')"
TRANSFER_SHA="$(sha256sum "${STAGE_ROOT}/d2_transfer_table.inc" | awk '{print $1}')"
MANIFEST="${STAGE_ROOT}/P3SM1R_LOGIN_MANIFEST.json"
python3 - "${MANIFEST}" "${REVISION}" "${DECK_SHA}" "${SOURCE_SHA}" "${TRANSFER_SHA}" <<'PY'
import json, sys
path, revision, deck, source, transfer = sys.argv[1:]
data = {
    "classification": "stage_p3sm1r_login_staging_complete",
...
if [ "${P3SM1R_PREFLIGHT_ONLY:-0}" = "1" ]; then
  echo "P3-SM1R preflight passed; submission intentionally skipped."
  exit 0
fi
...
if [[ ! "${JOB_ID}" =~ ^[0-9]+([.][A-Za-z0-9_-]+)?$ ]]; then
  echo "P3-SM1R submission returned invalid job ID; authorization remains unused: ${JOB_ID}" >&2
  exit 22
fi
python3 scripts/validation/consume_p3sm1r_authorization.py \
