...
if [ "${RETRY_AUTH}" = "true" ] || [ "${MPI_AUTH}" = "true" ] || [ "${THREAD_AUTH}" = "true" ] || [ "${HYBRID_AUTH}" = "true" ] || [ "${H1_AUTH}" = "true" ]; then
  echo "ERROR: Prohibited execution modes authorized in auth JSON (retry/mpi/threaded/hybrid/h1 must be false)."
  exit 1
fi

if [ "${SOLVER_AUTH}" != "true" ] || [ "${SOLVER_USED}" -ge "${MAX_SOLVER_SUB}" ] || [ "${SUBMISSION_APPROVED}" != "true" ] || [ "${ALLOW_SUBMIT}" != "1" ] || [ "${MAX_JOBS}" -lt 1 ]; then
...
python3 - "${MANIFEST}" "${REVISION}" "${STAGED_DECK_SHA}" "${STAGED_SOURCE_SHA}" "${RUNTIME_ROOT}" "${EXTRACTOR_SHA}" "${VALIDATOR_SHA}" <<'PY'
import json, sys
path, revision, deck, source, runtime_root, ext_sha, val_sha = sys.argv[1:]
json.dump(
    {
