#!/usr/bin/env bash
# Submit wrapper for Stage F Candidate Job A (Mode-II H2 uniform reference serial).
# Defaults to preflight-only unless --submit or MODE_II_H2_SOLVER_SUBMIT=1 is provided.
...
SUBMIT=false
if [ "${MODE_II_H2_SOLVER_SUBMIT:-0}" = "1" ]; then
  SUBMIT=true
fi
...
echo "Preflight check for Candidate Job A (Mode-II H2 uniform reference serial):"
...
PBS_SCRIPT="${SCRIPT_DIR}/mode_ii_h2_serial.pbs"
...
echo "Authorization verified. Submitting Candidate Job A (Mode-II H2 uniform reference)..."
