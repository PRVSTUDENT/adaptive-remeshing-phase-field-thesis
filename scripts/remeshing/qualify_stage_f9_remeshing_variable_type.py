...
def main():
    out = os.environ["F9_OUTPUT_DIRECTORY"]
    if not os.path.isdir(out):
        os.makedirs(out)
    odb_hash = sha256(os.environ["F9_SOURCE_ODB"])
    deck_hash = sha256(os.environ["F9_SOURCE_DECK"])
    ...
    Mdb()
    model = mdb.Model(name="F9_REMESH_TYPE")
    ...
    with open(os.path.join(out, "REMESHINGRULE_DOC_AUDIT.txt"), "w") as stream:
        ...
        stream.write("Symbolic constants: MODEL=%r UNIFORM_ERROR=%r NOT_ALLOWED=%r ON=%r\n"
                     % (MODEL, UNIFORM_ERROR, NOT_ALLOWED, ON))

    unicode_value = unicode("MISESERI")
    candidates = [
        ("unicode_tuple_control", (unicode_value,)),
        ("byte_string_tuple", ("MISESERI",)),
        ("ascii_encoded_tuple", (unicode_value.encode("ascii"),)),
        ("byte_string_list", ["MISESERI"]),
    ]
    attempts = []
    accepted = None
    for sequence, (label, variables) in enumerate(candidates, 1):
        if sequence > MAX_ATTEMPTS:
            break
        kwargs = dict(
            name="F9_%02d_%s" % (sequence, label),
        ...
        try:
            ...
            accepted = {
                ...
                "api_call": row["exact_api_call"],
                "repository_key": rule.name,
            }
    ...
    csv_path = os.path.join(out, "REMESHINGRULE_TYPE_MATRIX.csv")
    with open(csv_path, "wb") as stream:
        names = [
            "sequence", "label", "python_type", "nested_element_types", "repr",
            "exact_api_call", "exception", "rule_object_created",
        ]
    ...
    decision = {
        ...
        "accepted": accepted,
        "source_odb_sha256": odb_hash,
        "source_deck_sha256": deck_hash,
        "source_hashes_match": True,
        "installed_python_version": sys.version,
        "publication_faithful": {
            "variable": "MISESERI", "sizingMethod": "UNIFORM_ERROR",
            "errorTarget": 1.0, "coarseningFactor": "NOT_ALLOWED",
            "refinementFactor": 10,
        },
        "project_selected_sizes_mm": {"minimum": 0.001, "maximum": 0.010},
        "rule_object_created": bool(accepted),
        "solver_execution_count": 0,
        "native_adaptive_analysis_count": 0,
        "remesh_execution_count": 0,
        "candidate_deck_generated": False,
    }
    write_json(os.path.join(out, "REMESHINGRULE_TYPE_DECISION.json"), decision)
    write_json(os.path.join(out, "REMESH_RULE_MANIFEST.json"), {
        "classification": classification,
        "source_hashes_match": True,
        "rule_object_created": bool(accepted),
        "candidate_deck_generated": False,
...
try:
    ...
    output = os.environ.get("F9_OUTPUT_DIRECTORY", ".")
