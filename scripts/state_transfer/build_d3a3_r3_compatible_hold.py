#!/usr/bin/env python3
"""Build the separate D3A3-R3 compatible-state ingestion/release deck."""
...
DEFAULT_PACKAGE = Path("runs/hpc/stage_d3/interrupted_transfer/package_compatible_r1")
DEFAULT_D3A4 = Path("runs/hpc/stage_d3/interrupted_transfer/compatibility_projection_d3a4")
DEFAULT_R2_EXE = Path("models/state_transfer/d3_interrupted_transfer/executable")
DEFAULT_OUT = Path("models/state_transfer/d3_interrupted_transfer/executable_r3_compatible")
...
def generate_inp(path, model_dir, package_dir, d3a4_dir):
    ...
    active_rows = read_csv(d3a4_dir / "D3A4_ACTIVE_SET_BY_NODE.csv")
    ...
    lines = [
        "*Heading",
        "** D3A3-R3 compatible active-set release hold.",
        "** Compatible nodal d from D3A4; F1 H from package_compatible_r1.",
    ...
    lines += [
        "*Step, name=INGEST_COMPATIBLE_STATE, nlgeom=NO",
        "*Static",
        "1.0, 1.0",
        "*Boundary",
        "BOTTOM, 2, 2, 0.0",
    ...
    lines += [
        "*Step, name=CHECKPOINT_EQUILIBRATION_COMPATIBLE_FIXED, nlgeom=NO",
        "*Static",
        "0.1, 1.0, 1.0e-08, 0.1",
        "*Boundary",
        "BOTTOM, 2, 2, 0.0",
    ...
    lines += [
        "*Step, name=ACTIVE_SET_RELEASE_HOLD, nlgeom=NO",
        "*Static",
        "0.1, 1.0, 1.0e-08, 0.1",
        "*Boundary, op=NEW",
        "BOTTOM, 2, 2, 0.0",
...
def main():
    ...
    parser.add_argument("--d3a4-dir", type=Path, default=DEFAULT_D3A4)
    parser.add_argument("--r2-exe-dir", type=Path, default=DEFAULT_R2_EXE)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT)
    args = parser.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    src_fortran = args.r2_exe_dir / "d3_transfer_uel.for"
    dst_fortran = args.out_dir / "d3_transfer_uel.for"
    shutil.copyfile(src_fortran, dst_fortran)
    active = generate_inp(args.out_dir / "D3A3_R3_compatible_hold.inp", args.model_dir, args.package_dir, args.d3a4_dir)
    runtime_rows = write_csv_runtime(args.package_dir / "D3_TRANSFERRED_IP_H.csv", args.out_dir / "d3_transfer_h.dat")
    manifest = {
        "classification": "stage_d3a3_r3_runtime_h_state_generated",
    ...
    write_json(args.out_dir / "D3A3_R3_RUNTIME_MANIFEST.json", manifest)
    provenance = {
        "classification": "stage_d3a3_r3_input_prepared",
        "deck": str(args.out_dir / "D3A3_R3_compatible_hold.inp"),
        "compatible_package": str(args.package_dir),
        "d3a4_projection": str(args.d3a4_dir),
        "r2_fortran_source": str(src_fortran),
        "r3_fortran_source": str(dst_fortran),
        "r2_fortran_sha256": sha256(src_fortran),
        "r3_fortran_sha256": sha256(dst_fortran),
        "fortran_byte_identical_to_r2": sha256(src_fortran) == sha256(dst_fortran),
