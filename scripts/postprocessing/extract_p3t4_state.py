...
def extract(odb_path, deck_path, out_dir):
    ...
    write_csv(os.path.join(out_dir, "P3T4_STATE_OUTPUT.csv"), state_fields, rows)
    write_csv(
        os.path.join(out_dir, "P3T4_RF_U.csv"),
        ["frame", "step", "increment_number", "step_time", "U2", "RF2"],
        rf_rows,
    )
    write_csv(
        os.path.join(out_dir, "P3T4_ENERGY.csv"),
        ["frame", "step", "increment_number", "step_time", "ALLIE", "ALLSE", "ALLWK"],
        energy_rows,
    )
    odb.close()
    print(
        "p3t4_extract_ok elements=%d ips_per_element=%d frames=%d"
