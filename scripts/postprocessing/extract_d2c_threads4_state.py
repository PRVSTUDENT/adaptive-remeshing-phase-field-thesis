...
def extract(odb_path, package_dir, out_dir):
    ...
    write_csv(os.path.join(out_dir, "D2C_STATE_BY_FRAME.csv"), state_fields, rows)
    write_csv(
        os.path.join(out_dir, "D2C_RF_U.csv"),
        ["frame", "step", "increment_number", "step_time", "U2", "RF2", "ALLIE", "ALLSE", "ALLWK"],
        rf_rows,
    )
    with open(os.path.join(out_dir, "D2C_ENERGY_FRAMES.json"), "w") as handle:
        handle.write(json.dumps(energy_frames(rf_rows), indent=2, sort_keys=True) + "\n")
    odb.close()
    print("d2c_extract_ok records=%d frames=%d" % (len(rows), len(rf_rows)))


def main(argv=None):
    parser = argparse.ArgumentParser(description="Extract D2C four-thread continuation state from ODB")
