...
def energy_jumps(rf_rows):
    ...
    jumps = {"frames": energies, "jumps": {}}
    for name in ["ALLIE", "ALLSE", "ALLWK"]:
        f0 = energies.get("F0", {}).get(name)
        f2 = energies.get("F2", {}).get(name)
        f3 = energies.get("F3", {}).get(name)
        release_abs = None if f0 is None or f2 is None else f2 - f0
        continuation_abs = None if f2 is None or f3 is None else f3 - f2
        release_rel = None
        continuation_rel = None
        if release_abs is not None and f0 not in (None, 0.0):
            release_rel = release_abs / abs(f0)
        if continuation_abs is not None and f2 not in (None, 0.0):
            continuation_rel = continuation_abs / abs(f2)
        jumps["jumps"][name] = {
            "release_abs": release_abs,
            "release_rel": release_rel,
            "continuation_abs": continuation_abs,
            "continuation_rel": continuation_rel,
        }
    return jumps
...
def extract(odb_path, package_dir, out_dir):
    ...
    write_csv(os.path.join(out_dir, "D2B_STATE_BY_FRAME.csv"), state_fields, rows)
    write_csv(os.path.join(out_dir, "D2B_RF_U.csv"), ["frame", "step", "increment_number", "step_time", "U2", "RF2", "ALLIE", "ALLSE", "ALLWK"], rf_rows)
    with open(os.path.join(out_dir, "D2B_ENERGY_JUMP.json"), "w") as handle:
        handle.write(json.dumps(energy_jumps(rf_rows), indent=2, sort_keys=True) + "\n")
    odb.close()
    print("d2b_extract_ok records=%d frames=%d" % (len(rows), len(rf_rows)))


def main(argv=None):
    parser = argparse.ArgumentParser(description="Extract D2B continuation state from ODB")
