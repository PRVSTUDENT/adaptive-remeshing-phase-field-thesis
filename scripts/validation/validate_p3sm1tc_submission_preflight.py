#!/usr/bin/env python3
"""Fail-closed authorization and staging checks for P3-SM1TC."""
...
REQUIRED_FALSE = (
    "automatic_retry_authorized", "p3sm1r_authorized", "p3t4_authorized",
...
def validate_authorization(path: Path, require_submit: bool) -> dict[str, object]:
    ...
    expected = (
        "stage_p3sm1tc_thread_id_serial_authorized"
        if require_submit else "stage_p3sm1tc_thread_id_serial_prepared"
    )
    if data.get("classification") != expected:
        raise ValueError("unexpected authorization classification")
    if data.get("p3sm1tc_preparation_complete") is not True:
        raise ValueError("P3-SM1TC preparation incomplete")
    if data.get("maximum_p3sm1tc_submissions") != 1:
        raise ValueError("maximum_p3sm1tc_submissions must equal 1")
    used = data.get("p3sm1tc_submissions_used")
    if not isinstance(used, int) or isinstance(used, bool) or used != 0:
        raise ValueError("P3-SM1TC submission count consumed or invalid")
    if require_submit and data.get("p3sm1tc_submission_authorized") is not True:
        raise ValueError("P3-SM1TC submission not authorized")
...
def validate_manifest(path: Path, stage_root: Path) -> dict[str, object]:
    data = load_object(path)
    mapping = {
        "deck_sha256": stage_root / "P3SM1TC_serial.inp",
        "source_sha256": stage_root / "p3sm1tc_thread_id_callback.for",
...
def main() -> int:
    ...
    try:
        ...
        print(f"P3-SM1TC preflight blocked: {exc}")
        return 20
    print("P3-SM1TC preflight pass")
    return 0
