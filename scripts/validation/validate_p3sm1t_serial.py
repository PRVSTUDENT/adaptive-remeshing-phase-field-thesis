...
def validate(
    ...
    mapping = {
        "P3SM1T_ENVIRONMENT.txt": "P3SB_ENVIRONMENT.txt",
        "P3SM1T_JOB_RECORD.txt": "P3SB_JOB_RECORD.txt",
        "P3SM1T_STATE_OUTPUT.csv": "P3SB_STATE_OUTPUT.csv",
        "P3SM1T_RF_U.csv": "P3SB_RF_U.csv",
        "P3SM1T_ENERGY.csv": "P3SB_ENERGY.csv",
        "p3sm1t_serial.abaqus_stdout.log": "p3sb_baseline.abaqus_stdout.log",
        "p3sm1t_serial.sta": "p3sb_baseline.sta",
    }
    with tempfile.TemporaryDirectory(prefix="p3sm1t_baseline_gate_") as temporary:
    ...
    callback_path = out / "P3SM1T_CALLBACK_SUMMARY.json"
    if callback_path.is_file():
        ...
        failures.append("missing P3SM1T_CALLBACK_SUMMARY.json")
    ...
    if before != after or callback.get("unmatched_before_calls", 0) != 0:
        callback_failures.append("GETTHREADID before/after mismatch")
    ...
    if "GETTHREADID" not in source_text:
        callback_failures.append("GETTHREADID source missing")
    if "GETTHREADID()" not in routine_body(source_text, "UEL"):
        callback_failures.append("GETTHREADID call not inside UEL")
    if "GETTHREADID" in routine_body(source_text, "UEXTERNALDB"):
        callback_failures.append("GETTHREADID present inside UEXTERNALDB")
    if "GETTHREADID" in routine_body(source_text, "UMAT"):
        callback_failures.append("GETTHREADID present inside UMAT")
    ...
    if base["failures"]:
        classification = "stage_p3sm1t_threadid_serial_fail_validation"
    elif callback_failures:
        if (before and not after) or callback.get("signal_11_present") is True:
            classification = "stage_p3sm1t_threadid_serial_fail_identifier"
        else:
            classification = "stage_p3sm1t_threadid_serial_fail_callback"
    else:
        classification = "stage_p3sm1t_threadid_serial_pass"
    status = {
        **{key: value for key, value in base.items() if key not in ("classification", "P3SB_ok", "failures")},
        "classification": classification,
        "P3SM1T_ok": not failures,
    ...
    (out / "P3SM1T_INCREMENT_SEQUENCE.json").write_text(
        json.dumps(sequence, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (out / "P3SM1T_STATUS.json").write_text(
        json.dumps(status, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    marker = out / "P3SM1T_COMPLETION.ok"
    if marker.exists():
        marker.unlink()
    if not failures:
        marker.write_text(
            f"classification=stage_p3sm1t_threadid_serial_pass\njob_id={job_id}\n",
            encoding="utf-8",
        )
    return status
...
def main() -> int:
    ...
    return 0 if result["P3SM1T_ok"] else 1
