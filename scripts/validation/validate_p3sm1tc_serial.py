...
FORBIDDEN = (
    "GETRANK", "MUTEXINIT", "MUTEXLOCK", "MUTEXUNLOCK",
...
def validate(
    ...
    mapping = {
        "P3SM1TC_ENVIRONMENT.txt": "P3SB_ENVIRONMENT.txt",
        "P3SM1TC_JOB_RECORD.txt": "P3SB_JOB_RECORD.txt",
        "P3SM1TC_STATE_OUTPUT.csv": "P3SB_STATE_OUTPUT.csv",
        "P3SM1TC_RF_U.csv": "P3SB_RF_U.csv",
        "P3SM1TC_ENERGY.csv": "P3SB_ENERGY.csv",
        "p3sm1tc_serial.abaqus_stdout.log": "p3sb_baseline.abaqus_stdout.log",
        "p3sm1tc_serial.sta": "p3sb_baseline.sta",
    }
    with tempfile.TemporaryDirectory(prefix="p3sm1tc_baseline_gate_") as temporary:
    ...
    callback_path = out / "P3SM1TC_CALLBACK_SUMMARY.json"
    if callback_path.is_file():
        ...
        failures.append("missing P3SM1TC_CALLBACK_SUMMARY.json")
    ...
    returned = callback.get("returned_thread_ids", [])
    unique = callback.get("unique_thread_ids", [])
    ...
    if before != after or callback.get("unmatched_before_calls", 0) != 0:
        callback_failures.append("get_thread_id before/after mismatch")
    if not isinstance(returned, list) or not returned:
        callback_failures.append("returned thread-ID list empty")
    elif any(not isinstance(value, int) or isinstance(value, bool) or value < 0 or value >= 1
             for value in returned):
        callback_failures.append("returned thread ID outside OMP_NUM_THREADS=1")
    if unique != [0]:
        callback_failures.append("unique returned thread IDs must equal [0]")
    ...
    if "#INCLUDE <SMAASPUSERSUBROUTINES.HDR>" not in source_text:
        callback_failures.append("installed utility header include missing")
    if "GET_THREAD_ID()" not in routine_body(source_text, "UEL"):
        callback_failures.append("get_thread_id call not inside UEL")
    if "GET_THREAD_ID" in routine_body(source_text, "UEXTERNALDB"):
        callback_failures.append("get_thread_id present inside UEXTERNALDB")
    if "GET_THREAD_ID" in routine_body(source_text, "UMAT"):
        callback_failures.append("get_thread_id present inside UMAT")
    if "GETTHREADID()" in source_text.replace(" ", ""):
        callback_failures.append("undocumented GETTHREADID() present")
    runtime_text = ""
    for candidate in (
        out / "p3sm1tc_serial.abaqus_stdout.log",
        out / "p3sm1tc_serial.msg",
    ):
        if candidate.is_file():
            runtime_text += candidate.read_text(
                encoding="utf-8", errors="replace"
            ).lower()
    if "undefined symbol: getthreadid_" in runtime_text:
        callback_failures.append("unresolved legacy getthreadid_ symbol")
    if "undefined symbol: get_thread_id_" in runtime_text:
        callback_failures.append("unresolved documented get_thread_id_ symbol")
    ...
    if base["failures"]:
        classification = "stage_p3sm1tc_thread_id_serial_fail_validation"
    elif callback_failures:
        if (before and not after) or callback.get("signal_11_present") is True:
            classification = "stage_p3sm1tc_thread_id_serial_fail_identifier"
        else:
            classification = "stage_p3sm1tc_thread_id_serial_fail_callback"
    else:
        classification = "stage_p3sm1tc_thread_id_serial_pass"
    status = {
        **{key: value for key, value in base.items() if key not in ("classification", "P3SB_ok", "failures")},
        "classification": classification,
        "P3SM1TC_ok": not failures,
        ...
        "unique_thread_ids": unique,
        "unmatched_before_calls": callback.get("unmatched_before_calls", 0),
        "rank_thread_utilities_present": "GET_THREAD_ID" in source_text,
    ...
    (out / "P3SM1TC_INCREMENT_SEQUENCE.json").write_text(
        json.dumps(sequence, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (out / "P3SM1TC_STATUS.json").write_text(
        json.dumps(status, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    marker = out / "P3SM1TC_COMPLETION.ok"
    if marker.exists():
        marker.unlink()
    if not failures:
        marker.write_text(
            f"classification=stage_p3sm1tc_thread_id_serial_pass\njob_id={job_id}\n",
            encoding="utf-8",
        )
    return status
...
def main() -> int:
    ...
    return 0 if result["P3SM1TC_ok"] else 1
