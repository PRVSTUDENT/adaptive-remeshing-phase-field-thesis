#!/usr/bin/env python3
"""Static validation for the D3A3-R3 compatible active-set deck."""
...
def git_tracked(path):
    return subprocess.run(["git", "ls-files", "--error-unmatch", str(path)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0
...
def runtime_validate(path):
    ...
    return {
        "classification": "stage_d3a3_r3_runtime_state_validation_pass" if not failures else "stage_d3a3_r3_runtime_state_validation_fail",
...
def validate(args):
    ...
    active_rows = read_csv(args.d3a4_dir / "D3A4_ACTIVE_SET_BY_NODE.csv")
    ...
    if not (args.d3a4_dir / "D3A4.ok").exists() or not git_tracked(args.d3a4_dir / "D3A4.ok"):
    ...
    for node in active_nodes:
        ...
        if abs(float(row["d_compatible"]) - float(row["d_lb"])) > 1.0e-12:
            active_failures.append(node)
        if abs(compatible[node] - lower_d[node]) > 1.0e-12:
            active_failures.append(node)
    for node in free_nodes:
        row = active_by_node[node]
        if bool_value(row["active_lower_bound"]) or compatible[node] < lower_d[node] - 1.0e-12:
    ...
    step_bcs = parse_step_phase_bcs(args.exe_dir / "D3A3_R3_compatible_hold.inp")
    counts = {name: len(labels) for name, labels in step_bcs.items()}
    if counts.get("INGEST_COMPATIBLE_STATE") != N_NODES:
        failures.append("Step 1 phase BC count != 6601")
    if counts.get("CHECKPOINT_EQUILIBRATION_COMPATIBLE_FIXED") != N_NODES:
        failures.append("Step 2 phase BC count != 6601")
    if counts.get("ACTIVE_SET_RELEASE_HOLD") != EXPECTED_ACTIVE:
        failures.append("Step 3 phase BC count != 1880")
    step3 = set(step_bcs.get("ACTIVE_SET_RELEASE_HOLD", []))
    if step3 != set(active_nodes):
        failures.append("Step 3 phase BC labels do not match active set")
    if step3 & set(free_nodes):
        failures.append("Step 3 free-node phase BC count != 0")

    runtime = runtime_validate(args.exe_dir / "d3_transfer_h.dat")
    failures.extend("runtime: " + failure for failure in runtime["failures"])

    r2_fortran = args.r2_exe_dir / "d3_transfer_uel.for"
    r3_fortran = args.exe_dir / "d3_transfer_uel.for"
    source_hash_identical = sha256(r2_fortran) == sha256(r3_fortran)
    if not source_hash_identical:
        failures.append("UEL source hash differs from accepted R2 source")
    if (args.exe_dir / "d3_transfer_table.inc").exists():
        failures.append("obsolete d3_transfer_table.inc present")
    deck_text = (args.exe_dir / "D3A3_R3_compatible_hold.inp").read_text(encoding="utf-8", errors="replace")
    if "mp_mode=mpi" in deck_text.lower() or "mpi" in deck_text.lower():
        failures.append("MPI token present in deck")

    active_audit = {
        "classification": "stage_d3a3_r3_active_set_boundary_audit_pass" if not failures else "stage_d3a3_r3_active_set_boundary_audit_fail",
        ...
        "step3_free_node_phase_bc_count": len(step3 & set(free_nodes)),
        "step3_active_boundary_exact": step3 == set(active_nodes),
    }
    status = {
        "classification": "stage_d3a3_r3_static_validation_pass" if not failures else "stage_d3a3_r3_static_validation_fail",
        "D3A3_R3_static_ok": not failures,
        ...
        "source_hash_identical_to_r2": source_hash_identical,
        "r2_source_sha256": sha256(r2_fortran),
        "r3_source_sha256": sha256(r3_fortran),
        "obsolete_transfer_table_absent": not (args.exe_dir / "d3_transfer_table.inc").exists(),
        "mpi_absent": "mpi" not in deck_text.lower(),
        "failures": failures,
    }
    provenance = json.loads((args.exe_dir / "D3A3_R3_INPUT_PROVENANCE.json").read_text(encoding="utf-8"))
    args.out_dir.mkdir(parents=True, exist_ok=True)
    write_json(args.out_dir / "D3A3_R3_STATIC_VALIDATION.json", status)
    write_json(args.out_dir / "D3A3_R3_ACTIVE_SET_BOUNDARY_AUDIT.json", active_audit)
    write_json(args.out_dir / "D3A3_R3_RUNTIME_STATE_VALIDATION.json", runtime)
    write_json(args.out_dir / "D3A3_R3_INPUT_PROVENANCE.json", provenance)
    return status
...
def main():
    ...
    parser.add_argument("--exe-dir", type=Path, default=Path("models/state_transfer/d3_interrupted_transfer/executable_r3_compatible"))
    parser.add_argument("--r2-exe-dir", type=Path, default=Path("models/state_transfer/d3_interrupted_transfer/executable"))
    parser.add_argument("--out-dir", type=Path, default=Path("runs/hpc/stage_d3/interrupted_transfer/target_ingestion_r3_compatible_datacheck"))
    args = parser.parse_args()
    status = validate(args)
    print(json.dumps(status, indent=2, sort_keys=True))
    return 0 if status["D3A3_R3_static_ok"] else 1
