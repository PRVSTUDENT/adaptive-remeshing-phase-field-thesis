#!/usr/bin/env python3
"""Fail-closed static validator for Stage F36 M2RMBUILD10 qualification."""
...
def validate_f36_static_gate():
    failures = []

    # 1. Builder API check: writeInput(exactAssignment=True) prohibited, writeInput(consistencyChecking=ON) required
    builder_path = ROOT / "models/generated/mode_ii/f36_cae_import_key_repair/runtime/build_f36_geometry_backed_model.py"
    if not builder_path.exists():
        failures.append("F36 builder script build_f36_geometry_backed_model.py missing")
    else:
    ...
    pbs_path = ROOT / "models/generated/mode_ii/f36_cae_import_key_repair/M2RMBUILD10.pbs"
    if not pbs_path.exists():
        failures.append("M2RMBUILD10.pbs script missing")
    else:
        pbs_content = pbs_path.read_text(encoding="utf-8")
        if "M2RMBUILD10.pbs" not in pbs_content or 'cp "$F36_PACKAGE_DIR/M2RMBUILD10.pbs" .' not in pbs_content:
        ...
        if "abaqus cae noGUI=" in pbs_content and " -- " in pbs_content:
            failures.append("Prohibited '-- arguments' transport found in M2RMBUILD10.pbs")
        if "F36_SOURCE_DECK" not in pbs_content or "F36_OUTPUT_INPUT" not in pbs_content or "F36_GEOMETRY_AUDIT" not in pbs_content:
            failures.append("Required environment variables F36_SOURCE_DECK, F36_OUTPUT_INPUT, F36_GEOMETRY_AUDIT missing in PBS")
    ...
    if pbs_path.exists():
        ...
        if "on_exit" not in pbs_content or "trap on_exit EXIT" not in pbs_content:
            failures.append("Mandatory on_exit EXIT trap missing in M2RMBUILD10.pbs")
    ...
    if pbs_path.exists():
        pbs_content = pbs_path.read_text(encoding="utf-8")
        if "f36_m2rmbuild10_" in pbs_content and "STATUS.json" in pbs_content:
    ...
    orch_path = ROOT / "scripts/hpc/stage_f/submit_stage_f36_cae_build_qualification.sh"
    if not orch_path.exists():
        failures.append("Guarded orchestrator submit_stage_f36_cae_build_qualification.sh missing")
    else:
        orch_content = orch_path.read_text(encoding="utf-8")
        if "F36_ALLOW_SUBMISSION" not in orch_content or "F36_AUTHORIZE_M2RMBUILD10" not in orch_content:
...
if __name__ == "__main__":
    success = validate_f36_static_gate()
