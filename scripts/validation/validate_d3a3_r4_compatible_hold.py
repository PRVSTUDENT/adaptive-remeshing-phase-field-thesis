...
PASS_CLASSIFICATION = "stage_d3a3_r4_compatible_release_pass"
FAIL_CLASSIFICATION = "stage_d3a3_r4_active_set_release_fail"
KKT_FAIL_CLASSIFICATION = "stage_d3a3_r4_fixed_state_needs_reprojection"
...
def write_report(path: Path, status: dict):
    lines = [
        "# D3A3-R4 Compatible Hold Validation Report",
        "",
        "- Classification: `%s`" % status.get("classification"),
        "- D3A3_R4.ok: `%s`" % status.get("D3A3_R4_ok"),
...
def evaluate_active_free_gates(node_rows, expected_nodes, expected_active, expected_free):
    failures = []
    ...
    if f3_below_lb_violations != 0:
        failures.append("F3 below D3A5 lower-bound violations = %s" % f3_below_lb_violations)
    return {
        "failures": failures,
...
def evaluate_kkt_gates(kkt):
    ...
    if kkt.get("classification") == KKT_FAIL_CLASSIFICATION or not kkt.get("D3A3_R4_fixed_state_kkt_ok", False):
...
def validate(target_dir: Path, expected_active: int, expected_free: int, expected_nodes: int = 6601):
    failures = []
    required = [
        ...
        "D3A3_R4_ACTIVE_NODE_STATE.csv",
        "D3A3_R4_FREE_NODE_STATE.csv",
        "D3A3_R4_PHASE_NODE_STATE_BY_FRAME.csv",
        "D3A3_R4_LOWER_BOUND_AUDIT.json",
        "D3A3_R4_F1_KKT_METRICS.json",
        "D3A3_R4_F1_RESIDUAL_BY_NODE.csv",
    ]
    ...
    if failures:
        return {
            "classification": "stage_d3a3_r4_ingestion_fail",
            "failures": failures,
            "D3A3_R4_ok": False,
            "D3A3_ok": False,
        }
    ...
    phase_nodes = read_csv(target_dir / "D3A3_R4_PHASE_NODE_STATE_BY_FRAME.csv")
    lb_audit = read_json(target_dir / "D3A3_R4_LOWER_BOUND_AUDIT.json")
    kkt = read_json(target_dir / "D3A3_R4_F1_KKT_METRICS.json")
    ...
    if lb_audit.get("classification") != "stage_d3a3_r4_lower_bound_audit_pass":
    ...
    af = evaluate_active_free_gates(phase_nodes, expected_nodes, expected_active, expected_free)
    ...
    kkt_failed = (
        ...
        or not kkt.get("D3A3_R4_fixed_state_kkt_ok", False)
        or bool(kkt_eval["failures"])
    )
    if failures and kkt_failed:
...
def apply_status(target_dir: Path, status: dict):
    write_json(target_dir / "D3A3_R4_STATUS.json", status)
    write_report(target_dir / "D3A3_R4_REPORT.md", status)
    ok_r4 = target_dir / "D3A3_R4.ok"
    ok_all = target_dir / "D3A3.ok"
    if status.get("D3A3_ok"):
        ok_r4.write_text(PASS_CLASSIFICATION + "\n", encoding="utf-8")
...
def main():
    ...
    status = validate(args.target_dir, args.expected_active, args.expected_free, args.expected_nodes)
