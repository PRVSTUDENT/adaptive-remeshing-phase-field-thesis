...
from validate_p3t4_submission_preflight import REQUIRED_FALSE, validate_authorization
...
def consume(path: Path, job_id: str, revision: str) -> dict[str, object]:
    ...
    data.update({
        "classification": "stage_p3t4_threaded_characterization_submitted",
        "p3t4_submission_authorized": False,
        "p3t4_submissions_used": 1,
        "p3t4_job_id": job_id,
        "p3t4_submitted_revision": revision,
    })
...
def main() -> int:
    ...
    try:
        ...
        print(f"P3-T4 authorization consumption blocked: {exc}")
        return 20
    print(f"P3-T4 authorization consumed: {result['p3t4_job_id']}")
    return 0
