#!/usr/bin/env python3
"""Unit tests for Mode-II Evolving-Remesh / State-Transfer Restart Package (M2STATE_FRACFIX_RESTART1).
...
def test_package_manifest_integrity():
    ...
    assert raw["input_sha256"] == sha256_file(PKG_DIR / "M2STATE_FRACFIX_RESTART1.inp")
    assert raw["uel_sha256"] == sha256_file(PKG_DIR / "f42_mixed_uel.for")
    assert raw["pbs_sha256"] == sha256_file(PKG_DIR / "M2STATE_FRACFIX_RESTART1.pbs")
    assert raw["wrapper_sha256"] == sha256_file(PKG_DIR / "submit_m2state_fracfix_restart1.sh")


def test_state_transfer_artifact_gates():
    art = json.loads((PKG_DIR / "STATE_TRANSFER_ARTIFACT.json").read_text(encoding="utf-8"))
    assert art["transfer_validation_status"] == "PASS"
    assert art["phase_bound_violations"] == 0
    assert art["healing_count"] == 0
    assert art["energy_jump_pct"] <= 1.0
