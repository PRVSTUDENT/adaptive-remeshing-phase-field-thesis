...
parser = load("p3sm1tc_parser", "scripts/postprocessing/parse_p3sm1tc_callback_log.py")
preflight = load("p3sm1tc_preflight", "scripts/validation/validate_p3sm1tc_submission_preflight.py")
consumer = load("p3sm1tc_consumer", "scripts/validation/consume_p3sm1tc_authorization.py")
validator = load("p3sm1tc_validator", "scripts/validation/validate_p3sm1tc_serial.py")
PACKAGE = ROOT / "models/parallelization/p3sm1tc_thread_id_serial"
P3SM0 = ROOT / "models/parallelization/p3sm0_minimal_callback_serial"


def auth(**updates):
    data = {
        "classification": "stage_p3sm1tc_thread_id_serial_prepared",
        "p3sm1tc_preparation_complete": True,
        "p3sm1tc_submission_authorized": False,
        "maximum_p3sm1tc_submissions": 1,
        "p3sm1tc_submissions_used": 0,
        **{key: False for key in preflight.REQUIRED_FALSE},
    }
    data.update(updates)
    if data["p3sm1tc_submission_authorized"]:
        data["classification"] = "stage_p3sm1tc_thread_id_serial_authorized"
    return data
...
class ReferenceAndPackageTests(unittest.TestCase):
    def test_p3sm0_status_and_completion_unchanged(self):
        ...
        failed = ROOT / "runs/hpc/stage_p/p3sm1t_threadid_serial"
        closed = json.loads((failed / "P3SM1T_STATUS.json").read_text())
        self.assertEqual(
            closed["classification"],
            "stage_p3sm1t_threadid_serial_fail_identifier",
        )
        self.assertFalse((failed / "P3SM1T_COMPLETION.ok").exists())

    def test_deck_and_transfer_identity(self):
        self.assertEqual((PACKAGE / "P3SM1TC_serial.inp").read_bytes(),
    ...
    def test_only_installed_interface_hook_lines_added(self):
        base = (P3SM0 / "p3sm0_minimal_callback.for").read_text().splitlines()
        candidate = (PACKAGE / "p3sm1tc_thread_id_callback.for").read_text().splitlines()
        additions = [line for line in candidate if line not in base]
        self.assertEqual(additions, [
            "#include <SMAAspUserSubroutines.hdr>",
            "      INTEGER THREAD_ID",
            "        WRITE(7,*) 'P3SM1TC_BEFORE_GET_THREAD_ID'",
            "        THREAD_ID=get_thread_id()",
            "        WRITE(7,*) 'P3SM1TC_AFTER_GET_THREAD_ID ',THREAD_ID",
        ])

    def test_source_scope_and_forbidden_tokens(self):
        source = (PACKAGE / "p3sm1tc_thread_id_callback.for").read_text().upper()
        self.assertIn("#INCLUDE <SMAASPUSERSUBROUTINES.HDR>", source)
        self.assertIn("GET_THREAD_ID()", validator.routine_body(source, "UEL"))
        self.assertNotIn("GET_THREAD_ID", validator.routine_body(source, "UMAT"))
        self.assertNotIn("GET_THREAD_ID", validator.routine_body(source, "UEXTERNALDB"))
        self.assertNotIn("GETTHREADID()", source.replace(" ", ""))
        for token in validator.FORBIDDEN:
            self.assertNotIn(token, source)

    def test_fixed_form_column_72(self):
        lines = (PACKAGE / "p3sm1tc_thread_id_callback.for").read_text().splitlines()
    ...
    def test_lane_resources_ordering_and_no_compute_git(self):
        pbs = (ROOT / "scripts/hpc/stage_p/05_p3sm1tc_thread_id_serial.pbs").read_text()
        ...
        validation = pbs.index('validate_p3sm1tc_serial.py"')
...
class ParserTests(unittest.TestCase):
    ...
    def test_zero_and_multiple_zeros_pass_parser_shape(self):
        one = self.parse("P3SM1TC_BEFORE_GET_THREAD_ID\nP3SM1TC_AFTER_GET_THREAD_ID 0\n")
        self.assertEqual(one["unique_thread_ids"], [0])
        self.assertEqual(one["unmatched_before_calls"], 0)
        many = self.parse(("P3SM1TC_BEFORE_GET_THREAD_ID\nP3SM1TC_AFTER_GET_THREAD_ID 0\n") * 2)
    ...
    def test_missing_markers_mismatch_and_signal(self):
        before = self.parse("P3SM1TC_BEFORE_GET_THREAD_ID\nsignal 11\n")
        ...
        after = self.parse("P3SM1TC_AFTER_GET_THREAD_ID 0\n")
    ...
    def test_invalid_and_mixed_ids_are_recorded(self):
        for value in (-1, 1):
            result = self.parse(f"P3SM1TC_BEFORE_GET_THREAD_ID\nP3SM1TC_AFTER_GET_THREAD_ID {value}\n")
            self.assertEqual(result["returned_thread_ids"], [value])
        mixed = self.parse(("P3SM1TC_BEFORE_GET_THREAD_ID\nP3SM1TC_AFTER_GET_THREAD_ID 0\n"
                            "P3SM1TC_BEFORE_GET_THREAD_ID\nP3SM1TC_AFTER_GET_THREAD_ID 1\n"))
...
class ValidatorTests(unittest.TestCase):
    def fixture(self, root: Path):
        ...
        mapping = {
            "P3SM0_ENVIRONMENT.txt": "P3SM1TC_ENVIRONMENT.txt",
            "P3SM0_JOB_RECORD.txt": "P3SM1TC_JOB_RECORD.txt",
            "P3SM0_STATE_OUTPUT.csv": "P3SM1TC_STATE_OUTPUT.csv",
            "P3SM0_RF_U.csv": "P3SM1TC_RF_U.csv",
            "P3SM0_ENERGY.csv": "P3SM1TC_ENERGY.csv",
            "p3sm0_serial.abaqus_stdout.log": "p3sm1tc_serial.abaqus_stdout.log",
            "p3sm0_serial.sta": "p3sm1tc_serial.sta",
        }
        ...
        (root / "P3SM1TC_CALLBACK_SUMMARY.json").write_text(json.dumps(summary))

    def validate(self, root):
        return validator.validate(
            root, PACKAGE / "P3SM1TC_serial.inp",
            PACKAGE / "d2_transfer_table.inc",
            PACKAGE / "p3sm1tc_thread_id_callback.for", "synthetic", 0,
        )
    ...
    def test_complete_zero_id_fixture_passes_and_marks(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(result["P3SM1TC_ok"], result["failures"])
            self.assertTrue((root / "P3SM1TC_COMPLETION.ok").is_file())
    ...
    def test_marker_mismatch_signal_and_invalid_ids_fail(self):
        ...
        for mutation in mutations:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as td:
                root = Path(td)
                self.fixture(root)
                path = root / "P3SM1TC_CALLBACK_SUMMARY.json"
                data = json.loads(path.read_text())
                data.update(mutation)
                path.write_text(json.dumps(data))
                self.assertFalse(self.validate(root)["P3SM1TC_ok"])
                self.assertFalse((root / "P3SM1TC_COMPLETION.ok").exists())

    def test_missing_base_callback_and_scientific_evidence_fail(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.fixture(root)
            callback = root / "P3SM1TC_CALLBACK_SUMMARY.json"
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.fixture(root)
            (root / "P3SM1TC_ENERGY.csv").unlink()
            self.assertFalse(self.validate(root)["P3SM1TC_ok"])

    def test_legacy_unresolved_symbol_message_fails(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.fixture(root)
            path = root / "p3sm1tc_serial.abaqus_stdout.log"
            path.write_text(path.read_text() + "\nundefined symbol: getthreadid_\n")
            self.assertFalse(self.validate(root)["P3SM1TC_ok"])
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(ValueError):
                preflight.validate_authorization(
                    self.write(root, auth(p3sm1tc_submission_authorized=True,
                                          p3sm1tc_submissions_used=1)), True)

    def test_invalid_job_leaves_unused_valid_consumes_once(self):
        with tempfile.TemporaryDirectory() as td:
            path = self.write(Path(td), auth(p3sm1tc_submission_authorized=True))
            ...
            self.assertEqual(result["p3sm1tc_submissions_used"], 1)
            self.assertFalse(result["p3sm1tc_submission_authorized"])
            with self.assertRaises(ValueError):
                consumer.consume(path, "124.mmaster02", "a" * 40)

    def test_committed_successful_closure_and_downstream_false(self):
        data = json.loads((ROOT / "runs/hpc/stage_p/p3sm1tc_thread_id_serial/P3SM1TC_AUTHORIZATION.json").read_text())
        self.assertEqual(data["classification"], "stage_p3sm1tc_thread_id_serial_submitted")
        self.assertFalse(data["p3sm1tc_submission_authorized"])
        self.assertEqual(data["p3sm1tc_submissions_used"], 1)
        self.assertEqual(data["p3sm1tc_job_id"], "1378240.mmaster02")
