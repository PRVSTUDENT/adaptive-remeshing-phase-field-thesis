...
class TestF34M2RMBUILD9StaticGate(unittest.TestCase):

    def test_write_input_exactAssignment_absent(self):
        builder = ROOT / "models/generated/mode_ii/f34_cae_runtime_contract_repair/runtime/build_f34_geometry_backed_model.py"
        self.assertTrue(builder.exists(), "F34 builder script missing")
    ...
    def test_cae_environment_variable_argument_transport(self):
        builder = ROOT / "models/generated/mode_ii/f34_cae_runtime_contract_repair/runtime/build_f34_geometry_backed_model.py"
        pbs = ROOT / "models/generated/mode_ii/f34_cae_runtime_contract_repair/M2RMBUILD9.pbs"

        b_content = builder.read_text(encoding="utf-8")
        p_content = pbs.read_text(encoding="utf-8")

        self.assertIn("F34_SOURCE_DECK", b_content)
        self.assertIn("F34_OUTPUT_INPUT", b_content)
        self.assertIn("F34_GEOMETRY_AUDIT", b_content)

        self.assertIn("export F34_SOURCE_DECK=", p_content)
        self.assertIn("export F34_OUTPUT_INPUT=", p_content)
        self.assertIn("export F34_GEOMETRY_AUDIT=", p_content)
        self.assertNotIn("abaqus cae noGUI=runtime/build_f34_geometry_backed_model.py --", p_content, "No '-- arguments' transport allowed")

    def test_pbs_work_dir_self_staging(self):
        pbs = ROOT / "models/generated/mode_ii/f34_cae_runtime_contract_repair/M2RMBUILD9.pbs"
        content = pbs.read_text(encoding="utf-8")
        self.assertIn('cp "$F34_PACKAGE_DIR/M2RMBUILD9.pbs" .', content, "M2RMBUILD9.pbs must stage itself into WORK_DIR before SHA verification")
    ...
    def test_pbs_sha256sum_manifest_checks(self):
        ...
        self.assertIn("sha256sum -c F34_SHA256SUMS", content, "sha256sum -c F34_SHA256SUMS must be executed in PBS")
    ...
    def test_runtime_status_classifications(self):
        ...
        self.assertNotIn("f34_m2rmbuild9_static_clean_linux_qualified_not_authorized", content, "Authorization classification must not be in runtime STATUS.json")
