...
\texttt{SDV15} genuine-healing candidates by current script & 817 \\
...
The decision report assigns \texttt{paper\_matched\_v2\_scientific\_review\_incomplete}. The dominant RF--U mismatch is post-peak: pre-reference-peak RMSE is \(0.044136\ \mathrm{kN}\), post-reference-peak RMSE is \(0.348093\ \mathrm{kN}\), and full-overlap NRMSE is \(0.245705\). The final crack remains connected and approximately horizontal for \(\mathrm{SDV15}\) thresholds \(0.80\), \(0.90\), \(0.95\), and \(0.99\), with threshold-dependent extension from \(0.0555\ \mathrm{mm}\) down to \(0.0465\ \mathrm{mm}\). The retained \texttt{SDV15} artifacts preserve category counts and the worst drop, but not the raw decrease-event table needed for median, tolerance-bin, affected-point, frame-histogram, or equivalent-update-state decisions.

This is not a final scientific pass. Gate A3 remains \texttt{reference\_data\_insufficient} pending supervisor-approved tolerances, review of \texttt{SDV15} decreases and late-stage overshoot, and uniform-reference justification. The current authorization has been consumed by the one completed execution; retry authorization is \texttt{none}.
