...
This is preparation only. No retry has been submitted, no Abaqus run has been launched, and no scientific or phase-field validation claim follows. The next approval-sensitive action is the single one-CPU retry submission, not any benchmark, MISESERI, remeshing, state-transfer, parameter-study, production, or multi-CPU job.
...
\begin{itemize}
  ...
  \item HPC \texttt{UEXTERNALDB} callback invocation: unverified.
  \item Callback evidence investigation: \texttt{insufficient\_retained\_evidence}.
  \item Overall trivial user-subroutine smoke: failed.
  \item Deterministic callback retry: \texttt{prepared\_not\_submitted}.
