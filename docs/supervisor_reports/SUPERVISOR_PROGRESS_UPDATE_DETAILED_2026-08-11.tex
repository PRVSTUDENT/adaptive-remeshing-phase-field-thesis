...
H2$-$H1 matched RF / $d_{\max}$&--&--&$-1.445\%$ / $+0.001948$&at $u_{\max}^{\rm common}$\\
...
\clearpage\headx{3. Uniform-reference figures and interpretation (continued)}
\begin{center}
...
\item Qualify the corrected initialization path before any newly authorized execution, then verify source $\rightarrow$ artifact $\rightarrow$ first solver state.
...
\item No further H2 rerun: the extended calculation has answered the outstanding H2 termination question.
\end{enumerate}}
\end{minipage}
