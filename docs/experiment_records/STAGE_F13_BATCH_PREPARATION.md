...
The native-remesh package freezes the corrected 3,930-element CPE4 deck and official ODB hashes. Installed-API probing established the compliant direct call `model.adaptiveRemesh(odb)`. Job C therefore reuses the verified official ODB, executes zero source solvers and zero adaptivity-process submissions, calls one native remesh operation, and executes zero candidate solvers. A generated candidate may only be written and statically inspected.
