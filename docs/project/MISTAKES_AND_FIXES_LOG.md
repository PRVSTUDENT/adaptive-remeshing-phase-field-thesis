...
| M-059 | D3A3-R2 `1377389` | Runtime H loading avoided the R1 compile-token limit: Abaqus compiled and linked the user subroutine and completed input processing. Datacheck then failed because `UEXTERNALDB` opened relative `d3_transfer_h.dat` from Abaqus internal `/local/...` workdir where the file was not staged. | Next fix must make the runtime H file discoverable by Standard, for example by passing an absolute path to the user subroutine or staging the file into Abaqus' internal work directory before `UEXTERNALDB` opens it. Do not submit full D3A3-R2 before a compile/datacheck pass. | Open; no `D3A3_R2_COMPILE.ok` |
