...
| 2026-07-23 | D3A3-R2 `1377389.mmaster02` | Runtime H loading avoided the R1 compile-token limit: Abaqus compiled and linked the user subroutine and completed input processing. Datacheck then failed because `UEXTERNALDB` opened relative file `d3_transfer_h.dat` from Abaqus internal `/local/...` workdir where the file was not staged. | `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_compile_r2/` | Next fix should make the runtime H file discoverable by the Standard process, for example by passing an absolute path through the user subroutine or staging the file into Abaqus' internal work directory before `UEXTERNALDB` opens it. Do not submit full D3A3-R2 before a compile/datacheck pass. | Preserved as `stage_d3a3_r2_datacheck_fail_runtime_h_file_not_in_abaqus_workdir`; no `D3A3_R2_COMPILE.ok`. |
