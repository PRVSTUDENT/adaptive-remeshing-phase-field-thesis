...
| M-063 | D3A3-R2-R2 `1377393` | The counted-read datacheck compiled, linked, completed input processing and Standard datacheck, and printed `D3A3-R2 H LOAD COMPLETE 25600`, but PBS exited `10` and no compile marker was created. | Root cause was a workflow/pass-gate false negative: Abaqus wrapped the long GETOUTDIR path across two `.msg` lines (`...1377393.mmaster` and `02/d3_transfer_h.dat`), while the PBS gate exact-grepped the full canonical path on one line. Preserve failure evidence and patch future path confirmation to concatenate message lines after `D3A3-R2 H FILE PATH`, strip whitespace, and compare the reconstructed path to the expected canonical staged scratch path. Do not create `D3A3_R2_COMPILE.ok` from this job because PBS `Exit_status=10`. | Open; future retry requires explicit authorization |
