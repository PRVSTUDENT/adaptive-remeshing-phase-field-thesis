...
| M-076 | F43MODEREF6 governance | Historical tags `P43MODEREF6-FINAL2` and `Q43MODEREF6` violated anchor immutability rules: P tag was repeatedly force-pushed/recreated and Q tag pointed to the exact same commit as P (`9c8d267d7...`). | Preserve P6/Q6 as invalid historical anchors without deletion. Recover fresh immutable preparation tag `P43MODEREF7-FINAL1`, run exact-P detached qualification on `tu_freiberg` (619/619 tests pass, rc=0, natural cleanliness true), and create a separate provenance-only Q commit for `Q43MODEREF7-FINAL1`. | Resolved by F43MODEREF7-ANCHORRECOVERY1 |
...
| M-086 | P3-SB first cluster replay wrapper | The first combined SSH replay lost quoting around an inline Python fixed-form expression and was rejected at remote-shell parse time before pull or validation. | Split the replay into simple commands and make the fixed-form column gate a standard-library test rather than an inline cross-shell expression. | Resolved: qualified cluster suite passed; no Abaqus/PBS job was submitted |
| M-087 | P3-SB job `1378094.mmaster02` | Abaqus and extraction completed, but the validator could not see `p3sb_baseline.abaqus_stdout.log` or `.sta` because the PBS exit finalizer copied them from scratch only after validation. It consequently recorded false compile/link/input/completion gates and an empty increment sequence, returning scheduler exit `12`. | Preserve the committed failure unchanged and consume the one-shot authorization. Future lanes copy and verify all validator inputs before validation while retaining the finalizer as fallback; the P3-SM0 static suite enforces this order. | Original failure closed; offline finalized-evidence audit passed; prevention implemented in P3-SM0 preparation |
| M-088 | P3-SM0 synchronization | Two combined authorization/preflight shells stopped before submitter invocation: the first compared the full cluster HEAD with the abbreviation `572c51e`, and the second lost inline-Python quoting across PowerShell/SSH. A later closure-sync command fast-forwarded successfully but its final assertion used an incorrectly guessed full expansion of `6f1310a`. | Separate revision, authorization, queue, and submitter calls; avoid inline interpreted programs in cross-shell one-liners; obtain full SHAs directly with `git rev-parse` and never infer the remaining characters from an abbreviation. The single actual preflight and submission passed, and the closure replay used the observed full SHA. | Resolved; no failed wrapper consumed authorization or submitted a job |
| M-089 | P3-SM0 closure test | One preparation-era unit test required the committed authorization record to remain `prepared` at `0/1`, so it rejected the valid post-submission `submitted` state at `1/1`. | Update only the test expectation to require the exact consumed job ID, submitted revision, false submission flag, `1/1` count, and all downstream false flags. Do not alter the validator, submitter, scientific package, resources, or thresholds. | Resolved in closure validation |
...
## F1-J1-R2 Mode-II H0 serial scientific validation failure (2026-07-27)
...
Symptom: Abaqus FE solver completed cleanly (exit code 0, 16m 17s wallclock) and extractor completed cleanly (exit code 0), but result validator returned exit code 20 (`stage_f_mode_ii_h0_serial_validation_fail`).
Root cause: Step-2 in the H0 serial deck reached its maximum limit of 2000 increments of size 1.0e-4 mm ($U_1 = 0.0070\text{ mm}$) and completed, whereas the scientific acceptance gate checks for $U_1 = 0.0100\text{ mm}$. Maximum phase field reached $\max(d) = 0.29923 < 0.50$, so no crack-path points ($d \ge 0.50$) were produced.
Scientific inputs changed: no
Correction: Documented scientific validation failure and preserved partial pre-peak response evidence ($0 \le U_1 \le 0.0070\text{ mm}$, maximum observed force $RF_1 = 0.3063\text{ kN}$). Second replacement authorization is consumed (1/1); no retry or resubmission is authorized under protocol rules.
Retry job: none
Outcome: F1-J1-R2 failed scientific validation (`stage_f_mode_ii_h0_second_replacement_fail`). Baseline unvalidated; F2 and later tasks remain blocked.
