...
| M-062 | D3A3-R2-R2 correction | The EOF-driven loader necessarily issued one read after the validated 25,600th record. In the Abaqus/Intel `UEXTERNALDB` callback this post-last-record read surfaced as fatal end-of-file instead of a safe loop terminator. | Replace it with an exact counted loop and explicit premature-EOF/read-error branches. Preserve `GETOUTDIR`, the target deck, mesh/topology, transferred nodal `d`, transferred IP `H`, material properties, weak forms, layer offsets, step definitions, checkpoint U2, and runtime-H SHA256 `4689ea5c10c0972e69ba46f8676a326c8b011b98faa8031c7c26cfb218607cd9`. Submit only one serial counted-read datacheck after login-node reader smoke and commit/push. | Prepared; awaiting counted-read datacheck result |
