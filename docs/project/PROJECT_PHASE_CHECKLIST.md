...
| State-transfer gate | controlled and fracture transfer pass | D1 analytical pass, D2A Abaqus ingestion pass, D2B serial continuation pass, D2C four-thread repeatability pass, D2D0 external-tool audit complete, D3A accepted after scope-aware independent energy reconstruction, D3A2 nonmatching package pass, D3A3/R1/R2/R2-R1 failures preserved, D3A3-R2 compile/datacheck closed by deterministic postcheck replay, full D3A3-R2 job `1377396` preserved, ODB-only forensic replay classified corrected ingestion as `stage_d3a3_r2_ingestion_pass_release_not_accepted`, D3A4 offline constrained phase compatibility passed with `D3A4.ok` plus separate `package_compatible_r1`, failed R3 datacheck `1377404` preserved as pre-Abaqus staging failure, R1 staging-correction datacheck `1377409` passed with committed `D3A3_R3_DATACHECK.ok` under `target_ingestion_r3_compatible_datacheck_r1/` (M-069 resolved), and R3 full-hold scientific validation lane strengthened offline (active/free/lower-bound extract, F1 KKT analyzer, strengthened validator + synthetic gate tests; no full-hold submission) | D2D blocked by missing ABAQUSER; full D3A3-R3 compatible hold blocked pending explicit authorization after strengthened validator; downstream D3D/D3E fracture continuation remains blocked |
