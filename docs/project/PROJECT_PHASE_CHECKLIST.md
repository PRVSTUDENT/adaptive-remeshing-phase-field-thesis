...
- [~] D3A3-R1 compiler-environment correction prepared without changing physics, mesh, transfer values, input deck, Fortran logic, step definitions, or checkpoint displacement. R1 adds explicit `gcc/11.4.0`, `intel/2024.2.0`, and `abaqus/2023` module loads, `OMP_NUM_THREADS=1`, scratch execution, and environment/status capture under `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_r1/`.
