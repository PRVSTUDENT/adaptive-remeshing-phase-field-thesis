...
- [~] D3A3-R2 compile/datacheck gate prepared, not full-solver submitted. R2 replaces the compile-time Fortran DATA include with a headerless runtime `d3_transfer_h.dat` loaded once through `UEXTERNALDB`, while preserving the physical Molnar H0 UEL/UMAT source as the base. Local R2 evidence: `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_compile_r2/`, runtime validation classification `stage_d3a3_r2_runtime_state_validation_pass`, static validation classification `stage_d3a3_static_validation_pass`, 25600 H records, min H `1.0045788889553414e-08`, max H `0.027317036782803523`, SHA256 `4689ea5c10c0972e69ba46f8676a326c8b011b98faa8031c7c26cfb218607cd9`. Exactly one allowed next cluster action is `scripts/hpc/stage_d3/submit_d3a3_r2_compile_datacheck.sh`; no full D3A3-R2, D3D, or D3E job is allowed until `D3A3_R2_COMPILE.ok` exists and is committed.
