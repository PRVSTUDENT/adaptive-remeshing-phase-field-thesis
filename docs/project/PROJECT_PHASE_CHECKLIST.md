...
- [~] D3A3-R2-R1 pathfix compile/datacheck lane prepared locally, not yet accepted. The only generated-source change is replacing the relative runtime-H open with `GETOUTDIR` plus `HFILE(1:LHFILE)`; target mesh, transferred `d/H`, physical UEL/UMAT equations, input steps, material parameters, checkpoint displacement, and runtime H values are unchanged. Path audit classification: `stage_d3a3_r2_r1_path_audit_pass`; evidence `runs/hpc/stage_d3/interrupted_transfer/target_ingestion_compile_r2_r1/`; runtime H SHA256 remains `4689ea5c10c0972e69ba46f8676a326c8b011b98faa8031c7c26cfb218607cd9`. Exactly one corrected datacheck may be submitted via `scripts/hpc/stage_d3/submit_d3a3_r2_datacheck_pathfix.sh`; full D3A3-R2, D3D, and D3E remain blocked until `D3A3_R2_COMPILE.ok` exists and is committed.
